...
locals {
  ...
  sql_endpoint_connection_name = one([
    ...
    : connection.name if endswith(connection.properties.privateLinkServiceConnectionState.description, azurerm_data_factory_managed_private_endpoint.sql_adf_pe.var.adf_name)
  ])
}
