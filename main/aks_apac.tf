module "apac_gl_aks" {
  ...
  log_analytics_name      = var.aks_vars.azure_k8s_service.log_analytics_name_apac
  ...
  aks_subnet_id            = var.aks_vars.azure_k8s_service.aks_subnet_id_apac
  ...
  node_resource_group_name = var.aks_vars.azure_k8s_service.node_resource_group_name_apac
  # Network profile
  ...
  # Tags
  owner_tag        = var.apac_vars.tags.owner_tag
  account_tag      = var.apac_vars.tags.account_tag
  billingid_tag    = var.apac_vars.tags.billingid_tag
  costcenterit_tag = var.apac_vars.tags.costcenterit_tag
  sector_tag       = var.apac_vars.tags.sector_tag
  env_tag          = var.apac_vars.tags.env_tag
}